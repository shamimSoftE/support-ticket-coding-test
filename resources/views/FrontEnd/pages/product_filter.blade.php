                  ...
                  <form class="" method="get" action="{{ url('product-filter') }}">
                    ...
                    <div class="row mt-2">
                        ...
                        <div class="col-xxl-4 col-xl-4 col-lg-4 col-md-12 col-sm-12">
                            <div class="d-flex">
                                <input type="text" name="product_name" @if(isset($input['product_name'])) value="{{ $input['product_name'] }}"  @endif class="form-control" placeholder="Keyword">
                                &nbsp;<button class="btn btn-outline-success btn-sm" style="margin-left: -67px;" type="submit">Search</button>
                            </div>
                        </div>
                        <div class="col-xxl-2 col-xl-2 col-lg-2 col-md-12 col-sm-6">
                            <div class="d-flex">
                                <input type="text" class="form-control me-1" name="min_price" @if(isset($input['min_price'])) value="{{ $input['min_price'] }}"  @endif placeholder="$ min">

                                <input type="text" class="form-control" name="max_price" @if(isset($input['max_price'])) value="{{ $input['max_price'] }}"  @endif placeholder="$ max">
                            </div>
                        </div>

                        <div class="col-xxl-2 col-xl-2 col-lg-2 col-md-6 col-sm-12">
                            <div class="form-check">
                                <input class="form-check-input" name="auto_delivery" @if(isset($input['auto_delivery']) && $input['auto_delivery'] == 1) checked  @endif type="checkbox" value="1" id="auto_delivery" >
                                <label class="form-check-label" for="auto_delivery">
                                  Auto delivery
                                </label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" name="unlimited" type="checkbox" @if(isset($input['unlimited']) && $input['unlimited'] == 1) checked  @endif value="1" id="unlimited" >
                        ...
                        <div class="col-xxl-4 col-xl-4 col-lg-4 col-md-6 col-sm-12 d-flex">
                            ...
                            <select class="form-select form-select-sm me-2" aria-label="Default select example" name="low_high" style="height: 38px;">
                                <option value="">Select</option>
                                <option value="1" @if(isset($input['low_high']) && $input['low_high'] == 1) selected  @endif>Lowest Price</option>
                                <option value="2" @if(isset($input['low_high']) && $input['low_high'] == 2) selected  @endif>Highest Price</option>
                            </select>

                            {{-- <input class="form-control me-2" type="search" placeholder="Search" aria-label="Search"> --}}
                            <button class="btn btn-outline-success btn-sm" style="height: 38px;" type="submit">Search</button>
                        </div>
                    </div>
