    ...
    public function closeTicket(Request $request)
    {
        try{
            ...
            try {
                $mailData = [
                    'title' => 'Ticket Closed.',
                    'body' => $ticket->user->name.' Still have any question feel free to create a ticket.',
                    'footer' => 'Thanks for your time.',
                ];
                try {
                    Mail::to('with333us@gmail.com')->send(new SendMail($mailData));
                } catch (\Throwable $th) {
                //    dd($th->getMessage());
                }
            } catch (\Throwable $th) {
            ...
            $ticket->update($input);
            return redirect()->back()->with('success', 'The Ticket closed');
        }catch(\Throwable $e) {
