...
class OrderController extends Controller
{
        ...
        try {
            if($dayDiff <= 1)
            {
                ...
                if($cus_balance >= $product_price)
                {
                    ...
                    $input['total_amount'] = $product_price;
                    // dd($input);
    ...
    // vendor purchase
    public function venodrPurchase()
    {
        $vendor = User::where('deleted_at', null)->where('type', '1')->get();

       try {
            $vendor_purchase = [];
            foreach($vendor as $ven)
            {
                $order = Order::where('deleted_at', null)->where('customer_id', $ven->id)->get(['seller_id', 'product_id', 'id','customer_id','created_at','total_amount']);
                $vendor_purchase[] =  $order;
            }
            // dd($vendor_purchase);
            return view('FrontEnd.AdminPanel.vendor_purchase',compact('vendor_purchase'));
       } catch (\Throwable $th) {
        return redirect()->back();
       }
    }
