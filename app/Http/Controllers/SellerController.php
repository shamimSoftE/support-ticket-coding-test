...
class SellerController extends Controller
{
    ...
    public function sallerDashboard()
    {
        ...
        try {
            ...
            $data['order_process'] = Order::where('deleted_at', null)->where('seller_id',$user->id)->where('status', 0)->latest()->get();
            $data['order_disputes'] = Order::where('deleted_at', null)->where('seller_id',$user->id)->where('status', 3)->latest()->get();
        ...
        return view('FrontEnd.pages.seller.saller_dashboard',compact('user','data','withdraws','seller_balance','newsVendor','order_review','order_feedback','product_reviews','order_complete'));
    }
